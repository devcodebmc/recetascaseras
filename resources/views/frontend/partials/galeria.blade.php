  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="relative">
      ...
      <div class="absolute bottom-4 left-4 flex items-center space-x-2 bg-white bg-opacity-75 px-3 py-1 rounded-full shadow-md">
        <button 
          onclick="likeRecipe({{ $image->id }})" 
          class="text-red-400 hover:text-red-500 focus:outline-none like-button"
        ...
        <span class="text-gray-700 font-medium like-count" id="like-count-{{ $image->id }}">
          {{ $image->likes ?? 0 }}
        </span>
      </div>
    </div>
